...
Pauses tasks after consecutive failures with daily notification rate limiting.
Auto-recovers when login state/cookies are updated.
"""
...
class FailureGuard:
    ...
    Opens circuit after threshold failures, rate-limits daily notifications,
    and auto-recovers when cookie/login state is updated.
    """
    ...
    def record_success(self, task_key: str, *, now: Optional[datetime] = None) -> None:
        """Record successful task execution, resetting failure state."""
    ...
    def should_skip_start(
        ...
        *,
        ...
        """Check if task should skip starting due to being paused."""
        current = _now(self.tz_name, now=now)
    ...
    def record_failure(
        ...
        *,
        ...
        """Record task failure, potentially opening the circuit."""
        current = _now(self.tz_name, now=now)
        today = _today_str(self.tz_name, now=current)
