"""
Seller profile cache with TTL expiration and request coalescing.
Avoids redundant fetches when multiple requests arrive for the same seller.
"""
import asyncio
import copy
import time
...
class SellerProfileCache:
    """
    Cache for seller profiles with TTL expiration and concurrent request coalescing.
    ...
    async def get_or_load(self, user_id: str, loader: SellerProfileLoader) -> dict:
        """
        Get seller profile from cache or load via loader function.
        Concurrent requests for the same user_id will share a single load.
        """
