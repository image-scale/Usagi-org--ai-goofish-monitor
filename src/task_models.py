"""
Task domain models with Pydantic validation.
Defines Task, TaskCreate, TaskUpdate, and TaskGenerateRequest.
"""
import re
...
from src.cron_utils import validate_cron_expression
from src.account_strategy import clean_account_state_file, normalize_account_strategy
...
def _normalize_keyword_values(value) -> List[str]:
    """Normalize keyword values: split, trim, deduplicate."""
    if value is None:
        return []
...
def _extract_keywords_from_legacy_groups(groups) -> List[str]:
    """Flatten legacy keyword_rule_groups to keyword_rules."""
    if not groups:
        return []
...
def _normalize_payload_keywords(payload: Any) -> Any:
    """Normalize payload keywords and account settings."""
...
class Task(BaseModel):
    ...
    def apply_update(self, update: "TaskUpdate") -> "Task":
        """Apply update and return new task instance."""
...
class TaskCreate(BaseModel):
    ...
    def validate_decision_mode_payload(self):
        ...
        if self.decision_mode == "ai" and not description:
            raise ValueError("AI mode requires description.")
        ...
        if self.account_strategy == "fixed" and not self.account_state_file:
            raise ValueError("Fixed account strategy requires account_state_file.")
        return self
...
class TaskGenerateRequest(BaseModel):
    """Task generation request DTO (supports AI criteria generation)."""
