"""Tests for search pagination handler."""
import asyncio

from src.search_pagination import (
    advance_search_page,
...
def test_advance_search_page_stops_when_no_next_button():
    ...
    assert "no usable" in logs[0].lower()


def test_advance_search_page_stops_after_timeout_retries():
    page = FakePage(
        next_button_count=1,
        outcomes=[
...
def test_advance_search_page_returns_response_on_success():
...
def test_advance_search_page_stops_when_click_times_out():
    page = FakePage(
        next_button_count=1,
...
def test_is_search_results_response_matches_exact_search_api():
    response = FakeResponse(
...
def test_is_search_results_response_rejects_shade_api():
    response = FakeResponse(
...
def test_is_search_results_response_rejects_non_post():
    response = FakeResponse(
        url="https://h5api.m.goofish.com/h5/mtop.taobao.idlemtopsearch.pc.search/1.0/?foo=bar",
        method="GET",
    )
